...
def _select_voice_ids(user_id: int, voice_override: Optional[str]) -> tuple[str, str]:
    """
    Returns a tuple: (chatgpt_voice, elevenlabs_voice).
    chatgpt_voice is preferred; elevenlabs_voice used only if ELEVENLABS_API_KEY is set
    and USE_ELEVENLABS=1.
    """
    pref = _voice_pref_for_user(user_id)
    if voice_override:
        # If override is not a valid ChatGPT voice and we are NOT forcing ElevenLabs,
        # ignore it for ChatGPT but still allow it for EL when enabled.
        use_el = os.getenv("USE_ELEVENLABS", "0") == "1"
        if not use_el and voice_override not in _VALID_CHATGPT_VOICES:
            pass  # let normal prefs decide for ChatGPT; keep override for EL path only
        else:
            return voice_override, voice_override
    ...
    return chatgpt_voice, el_voice


def _select_azure_voice(user_id: int, voice_override: Optional[str]) -> Optional[str]:
    """
    Choose Azure voice: override > gender pref (male/female envs) > AZURE_SPEECH_VOICE > default.
    """
    pref = _voice_pref_for_user(user_id)
    if voice_override:
    ...
    voice_male = os.getenv("AZURE_SPEECH_VOICE_MALE") or os.getenv("AZURE_TTS_VOICE_MALE")
    voice_female = os.getenv("AZURE_SPEECH_VOICE_FEMALE") or os.getenv("AZURE_TTS_VOICE_FEMALE")
    if pref == "male" and voice_male:
        return voice_male
    if pref == "female" and voice_female:
        return voice_female
...
def generate_podcast_audio_for_voice(
    ...
    """
    audio_bytes = None
    chatgpt_voice, el_voice = _select_voice_ids(user_id, voice_override)
    azure_voice = _select_azure_voice(user_id, voice_override)
    use_el = os.getenv("USE_ELEVENLABS", "0") == "1"
