...
class User(Base):
    ...
    role         = Column(String(32), nullable=False, server_default=text("'user'"))  # superuser|admin|coach|user
    allow_raw    = Column(Boolean, nullable=False, server_default=text("false"))
...
class UserVisibility(Base):
    ...
    __table_args__  = (
        UniqueConstraint("tenant_id", "viewer_user_id", "target_user_id", name="uq_visibility_tenant_viewer_target"),
    )
...
class Report(Base):
    ...
    tenant_id   = Column(Integer, ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False, index=True)
    user_id     = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    run_id      = Column(Integer, ForeignKey("assessment_runs.id", ondelete="SET NULL"), nullable=True, index=True)
    masked      = Column(Boolean, nullable=False, server_default=text("true"))
    storage_path= Column(String(255), nullable=False)
    created_by  = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True, index=True)
    created_at  = Column(DateTime, default=datetime.utcnow, nullable=False)
    __table_args__ = (
        Index("ix_reports_tenant_user", "tenant_id", "user_id"),
    )
...
class ReportAccessLog(Base):
    ...
    target_user_id = Column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False, index=True)
    run_id         = Column(Integer, ForeignKey("assessment_runs.id", ondelete="SET NULL"), nullable=True, index=True)
    mode           = Column(String(16), nullable=False)  # aggregates|masked|raw
    ...
    __table_args__ = (
        Index("ix_report_access_tenant_viewer", "tenant_id", "viewer_id", "accessed_at"),
    )
