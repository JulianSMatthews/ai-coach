...
def send_boost(user: User, coach_name: str = COACH_NAME, week_no: int | None = None) -> None:
    ...
    if log_llm and transcript:
        print(f"[prompts] logged podcast_friday prompt for user_id={user.id}")
    ...
    if audio_url:
        ...
        try:
            send_whatsapp_media(to=user.phone, media_url=audio_url, caption=message)
        except Exception:
            send_whatsapp(to=user.phone, text=f"{message} {audio_url}")
    else:
