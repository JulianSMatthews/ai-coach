...
def send_tuesday_check(user: User, coach_name: str = COACH_NAME) -> None:
    ...
    with SessionLocal() as s:
        ...
        if not wf:
            _send_tuesday(user, text="Your weekly plan is still being prepared. Please try again shortly.")
            return
        ...
        if not kr:
            _send_tuesday(user, text="No key results found for this week. Say monday to set them up.")
            return
