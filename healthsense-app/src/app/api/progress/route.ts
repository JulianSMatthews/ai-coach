...
export async function GET(request: Request) {
  try {
    ...
    const upstream = `${base}/api/v1/users/${encodeURIComponent(userId)}/progress${params.toString() ? `?${params.toString()}` : ""}`;
    ...
    if (!res.ok) {
      return NextResponse.json({ error: text || "Failed to load progress" }, { status: res.status });
    }
