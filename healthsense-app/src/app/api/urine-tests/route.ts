...
export async function GET(request: Request) {
  try {
    ...
    const upstream = `${base}/api/v1/users/${encodeURIComponent(userId)}/urine-tests/latest`;
    ...
    if (!res.ok) {
      return NextResponse.json({ error: text || "Failed to load urine test" }, { status: res.status });
    }
...
export async function POST(request: Request) {
  try {
    ...
    const payload = {
      captured_at: String(body.capturedAt || new Date().toISOString()),
      capture_stage: String(body.captureStage || "single"),
      file_name: String(body.fileName || ""),
      mime_type: String(body.mimeType || ""),
      size_bytes: Number(body.sizeBytes || 0),
      image_data_url: typeof body.imageDataUrl === "string" ? body.imageDataUrl : "",
    };
    const base = getBaseUrl();
    const upstream = `${base}/api/v1/users/${encodeURIComponent(userId)}/urine-tests/photo`;
    const res = await fetch(upstream, {
      method: "POST",
    ...
    const text = await res.text().catch(() => "");
    if (!res.ok) {
      return NextResponse.json({ error: text || "Failed to save urine test photo" }, { status: res.status });
    }
    return NextResponse.json(text ? JSON.parse(text) : {});
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
