...
export async function POST(request: Request) {
  try {
    ...
    const res = await fetch(`${base}/api/v1/users/${encodeURIComponent(userId)}/assessment/chat/tracker-summary`, {
      method: "POST",
      headers,
      cache: "no-store",
    });
    ...
    if (!res.ok) {
      return NextResponse.json({ error: text || "Failed to load Gia's message" }, { status: res.status });
    }
    if (!text) {
