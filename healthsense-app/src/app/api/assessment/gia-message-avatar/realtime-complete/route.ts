...
export async function POST(request: Request) {
  try {
    ...
    const res = await fetch(`${base}/api/v1/users/${encodeURIComponent(userId)}/assessment/gia-message-avatar/realtime-complete`, {
      method: "POST",
      headers,
    ...
    if (!res.ok) {
      const errorMessage =
        res.status >= 500
          ? "We couldn't log Gia's video session right now."
          : text || "Failed to complete Gia avatar video session";
