...
function buildUrl(base: string, userId: string, usePublic = false) {
  ...
  return `${base}${prefix}/${encodeURIComponent(userId)}/assessment/summary-avatar/realtime-complete`;
}
...
export async function POST(request: Request) {
  try {
    ...
    const res = await forwardWithFallbacks(base, userId, session, JSON.stringify(body));
    if (!res) {
      return NextResponse.json({ error: "We couldn't log the summary video session right now." }, { status: 502 });
    }
    ...
    if (!res.ok) {
      ...
      const errorMessage =
        res.status >= 500
          ? "We couldn't log the summary video session right now."
          : text || "Failed to complete realtime summary avatar session";
