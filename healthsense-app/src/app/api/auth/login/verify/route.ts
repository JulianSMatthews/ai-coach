...
export async function POST(request: Request) {
  ...
  let res: Response;
  try {
    res = await fetch(`${base.replace(/\/+$/, "")}/api/v1/auth/login/verify`, {
      method: "POST",
  ...
  if (!res.ok) {
    return NextResponse.json({ error: extractUpstreamMessage(text) || "Failed to verify OTP" }, { status: res.status });
  }
  if (!text.trim()) {
