...
export async function POST(request: Request) {
  ...
  let res: Response;
  try {
    res = await fetch(`${base.replace(/\/+$/, "")}/api/v1/auth/register/verify`, {
      method: "POST",
  ...
  if (!res.ok) {
    return NextResponse.json({ error: extractUpstreamMessage(text) || "Failed to verify account code" }, { status: res.status });
  }
  if (!text.trim()) {
