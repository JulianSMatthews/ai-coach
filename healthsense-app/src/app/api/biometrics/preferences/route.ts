...
export async function POST(request: Request) {
  try {
    ...
    const upstream = `${base}/api/v1/users/${encodeURIComponent(userId)}/wearables/biometrics/preferences`;
    ...
    if (!res.ok) {
      return NextResponse.json({ error: text || "Failed to save biometrics preferences" }, { status: res.status });
    }
