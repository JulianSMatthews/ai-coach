...
export function friendlyAuthError(raw: unknown): string {
  ...
  if (lower.includes("invalid credentials")) return "That password didn’t match. Try again.";
  if (lower.includes("password required")) return "Enter your password to continue.";
  if (lower.includes("channel must be")) return "Choose WhatsApp or SMS.";
  if (lower.includes("failed to send otp")) return "We couldn’t send a code. Try again or choose SMS.";
